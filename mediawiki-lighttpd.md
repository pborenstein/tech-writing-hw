# How to Install MediaWiki and an LLMP (Linux, Lighttpd, MySQL, and PHP) Stack
...
We'll install and test each component one at a time. There are four main sections to this tutorial:
...
Since this is our first time using `apt` on this server, we start off by updating our local package index. We can then install `lighttpd`. The `--yes` option assumes a "yes" answer to any prompts.
...
```command
sudo apt-get --yes install php5-fpm php5-mysql
```
...
```command
cp 15-fastcgi-php.conf 15-fastcgi-php.conf.original
```
...
```command
sudo apt-get --yes install php5-intl
```

`php5-gd` provides support for creating image thumbnails.

```command
sudo apt-get --yes install php5-gd
```

`php5-xcache` caches pages so they don't have to be rerendered if there are no changes.

```commmand
sudo apt-get --yes install php5-xcache
```
...
Restart `lighttpd` to pick up the new modules.
...
$$$CLEANUP too many words that mean different things.

In this step, you'll install the MySQL database package. MediaWiki uses a database to store its contents and to manage wiki users. You can use other databases such as SQLite or PostgreSQL.
...
Next, clean up and secure MySQL. The `mysql_secure_installation` scripts asks you several questions. Since you already set up a `root` password when you installed MySQL, you can answer `no` for that question, and answer `yes` (the default) for all the other questions.
...
Next, create a database user to work with the `wikidb` database.  This is a long command, so we'll split it into three lines. The prompt will change to `->` to indicate that you haven't finished entering the command.
...
Use the name you chose for the database instead of `<^>wikidb<^>`. You can use any name and password for the database user `<^>sammy<^>` and for `<^>password<^>`. Be sure to remember these three values. You will need them when you install MediaWiki.
...
The first page lets you choose the language the installer should use during installation and the main language of the wiki itself. The default for both is English.

Click **Continue**.

The next page checks whether MediaWiki can run on your machine. Look for green text that says "The environment has been checked. You can install MediaWiki".
...
You can select **I'm bored already, just install the wiki**, but if you want to enable some options such as file uploading, select **Ask me more questions**, and click **Continue**.

Each section in the **Options** page has help text that describes what the options do. Click **Continue** to finish setting up the options.
...
To learn more about managing a MediaWiki installation, see [MediaWiki System Administration Manual](https://www.mediawiki.org/wiki/Manual:System_administration).
